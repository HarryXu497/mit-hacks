...
## Canopy Clash — geometric jungle edition

This workspace adds a procedural jungle scene to the original Bevy game: blocky monkey players, grass stripes, timber goals, a scoreboard shrine, faceted foliage, bridges, huts, and animated water. Run `cargo run --bin cube-soccer`, or on Windows `./run-jungle.ps1`. See [running and capture instructions](docs/RUNNING.md).

The original README follows below. Its RL-ready claims describe intended functionality: the upstream Rust RL environment is still a placeholder and is not made trainable by this visual pass.
