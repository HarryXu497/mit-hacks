...
const FONT: &[u8] = include_bytes!("../../assets/fonts/MPLUSRounded1c-Black.ttf");
...
/// The parent of the whole wordmark, carrying the entrance animation clock.
#[derive(Component)]
pub struct WordmarkRoot {
    age: f32,
}
...
/// Where the wordmark stands in the world.
///
/// Not parented to the camera: it is a thing in the scene, so it holds still
/// while the title drift moves around it and the monkeys on it cast shadows
/// onto the cliff behind. Placed over the empty hillside left of the bowl,
/// which is the part of frame the title shot deliberately keeps clear.
pub fn anchor() -> Transform {
    // Turned to face roughly down the barrel of the title shot. Not square to
    // it: a few degrees off keeps the side walls of the extrusion visible,
    // which is the only thing that says the letters are solid.
    Transform::from_xyz(-41.5, 16.5, 20.5)
        .with_rotation(Quat::from_rotation_y(-0.25) * Quat::from_rotation_x(-0.05))
}
...
pub fn spawn(
    ...
    let root = commands
        .spawn((
            ...
            WordmarkRoot { age: 0. },
            NoMerge,
            SpatialBundle::from_transform(anchor()),
        ))
        .id();

    for (index, text) in ["CANOPY", "CLASH"].iter().enumerate() {
...
mod tests {
    ...
    fn font() -> Vec<u8> {
        std::fs::read("assets/fonts/MPLUSRounded1c-Black.ttf").expect("title font")
    }

    #[test]
    fn a_line_extrudes_to_the_requested_height_and_has_depth() {
        let (mesh, width) = line_mesh(&font(), "CANOPY", LINE_HEIGHT).expect("mesh");
        let positions = mesh
            .attribute(Mesh::ATTRIBUTE_POSITION)
            .unwrap()
            .as_float3()
    ...
    fn the_ink_shell_encloses_the_letters_it_outlines() {
        let (mesh, _) = line_mesh(&font(), "CLASH", LINE_HEIGHT).expect("mesh");
    ...
    fn both_lines_of_the_wordmark_build() {
        for text in ["CANOPY", "CLASH"] {
            assert!(line_mesh(&font(), text, LINE_HEIGHT).is_some(), "{text} failed");
        }
    }
}

/// Builds the wordmark when the title card opens.
///
/// Safe to run here precisely because it is not `Startup`: the scenery merge in
/// `rendering::batching` absorbs static meshes and despawns the originals at
/// `PostStartup`, so anything spawned before it silently stops being drawn
/// while still reporting itself visible. The `NoMerge` tags are belt and braces
/// for the day someone moves this earlier.
pub fn spawn_wordmark(
    mut commands: Commands,
    mut meshes: ResMut<Assets<Mesh>>,
    mut materials: ResMut<Assets<StandardMaterial>>,
) {
    spawn(&mut commands, &mut meshes, &mut materials, FONT);
}

pub fn despawn_wordmark(
    mut commands: Commands,
    roots: Query<Entity, (With<Wordmark>, Without<Parent>)>,
) {
    for root in &roots {
...
/// The entrance, and then a slow breath.
///
/// Same overshoot curve as the flat title used, so the wordmark still arrives
/// by being thrown at the screen rather than by fading up. Once settled it
/// drifts a fraction of a degree, which stops three tonnes of geometry from
/// looking welded to the hillside.
pub fn animate_wordmark(
    time: Res<Time>,
    mut roots: Query<(&mut Transform, &mut WordmarkRoot)>,
) {
    for (mut transform, mut root) in &mut roots {
        root.age += time.delta_seconds();
        let t = (root.age / 0.55).clamp(0., 1.);
        let overshoot = 1. - (1. - t).powi(3) * (t * 9.4).cos();
        let base = anchor();
        transform.scale = Vec3::splat(0.55 + 0.45 * overshoot);
        let sway = (root.age * 0.5).sin() * 0.012 * t;
        transform.rotation = base.rotation * Quat::from_rotation_z(sway);
        transform.translation = base.translation + Vec3::Y * (root.age * 0.7).sin() * 0.06 * t;
    }
}
